package org.cxyxh.blogserver.utils;

import org.apache.poi.hpsf.DocumentSummaryInformation;
import org.apache.poi.hpsf.SummaryInformation;
import org.apache.poi.hssf.usermodel.*;
import org.apache.poi.ss.usermodel.FillPatternType;
import org.apache.poi.ss.usermodel.IndexedColors;
import org.apache.poi.ss.util.CellRangeAddress;
import org.cxyxh.blogserver.model.UserDataDownload;
import org.springframework.http.HttpHeaders;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.http.ResponseEntity;

import java.io.ByteArrayOutputStream;
import java.io.IOException;
import java.util.Date;
import java.util.List;

/**
 * @ProjectName: blog
 * @Package: org.cxyxh.blog.utils
 * @ClassName: POIUtils
 * @Author: Administrator
 * @Description: ${description}
 * @Date: 2020/5/28 10:36
 * @Version: 1.0
 */
public class POIUtils {


   /* 1.一个sheet如何设置多种样式
   *  2.把时间的问题修正一下
   *  3.把列宽调整一下
   *  4.针对表格的样式做调整
   *
   * */


    public static ResponseEntity<byte[]> userDataDownload(List<UserDataDownload> data, Date[] createDateScope) {
        //1.创建一个excel文档
        HSSFWorkbook workbook = new HSSFWorkbook();
        //生成文档基本信息
        genExcelBaseInfo(workbook, createDateScope);
        // 生成数据
        genExcelSheet(workbook, data, createDateScope);
        // 数据格式转换
        ResponseEntity<byte[]> responseEntity = dataFormatConversion(workbook, createDateScope);

        return responseEntity;
    }

    /**
     * 生成文档基本信息
     *
     * @param workbook
     * @param createDateScope
     */
    public static void genExcelBaseInfo(HSSFWorkbook workbook, Date[] createDateScope) {
        //2.创建文档摘要
        workbook.createInformationProperties();
        //3.获取并配置文档信息
        DocumentSummaryInformation docInfo = workbook.getDocumentSummaryInformation();
        //3.1文档类别 文档管理员 公司
        docInfo.setCategory("用户访问数据表");
        docInfo.setManager("程序员小黄");
        docInfo.setCompany("org.cxyxh");
        //4.获取文档摘要信息
        SummaryInformation summInfo = workbook.getSummaryInformation();
        //4.1 文档标题 文档备注
        summInfo.setTitle("用户访问数据表");
        summInfo.setComments("本文档由 程序员小黄 提供");
    }

    /**
     * 生成sheet
     *
     * @param workbook
     * @param data
     * @param createDateScope
     */
    private static void genExcelSheet(HSSFWorkbook workbook, List<UserDataDownload> data, Date[] createDateScope) {
        HSSFSheet sheet = workbook.createSheet("用户访问数据表");
        //0.设置宽度
        genExcelColumWidth(sheet);
        //1.创建标题行
        genExcelTitleRow(workbook, sheet, createDateScope);
        //2.创建表头行
        genExcelHeaderRow(workbook, sheet);
        //3.创建数据行
        genExcelDataRow(workbook, sheet, data);
    }

    /**
     * 设置excel表格宽度
     *
     * @param sheet
     */
    private static void genExcelColumWidth(HSSFSheet sheet) {
        sheet.setColumnWidth(0, 5 * 256);
        sheet.setColumnWidth(1, 10 * 256);
        sheet.setColumnWidth(2, 15 * 256);
    }

    /**
     * 创建标题行数据
     *
     * @param workbook
     * @param sheet
     */
    public static void genExcelTitleRow(HSSFWorkbook workbook, HSSFSheet sheet, Date[] createDateScope) {
        //设置标题样式
        HSSFCellStyle titleStyle = workbook.createCellStyle();
//        titleStyle.setFillBackgroundColor(IndexedColors.YELLOW.index);
        titleStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        //设置标题数据
        HSSFRow r0 = sheet.createRow(0);
        HSSFCell c0 = r0.createCell(0);
        String title = createDateScope[0] + "_" + createDateScope[1] + " 用户访问数据表";
        c0.setCellValue(title);
        c0.setCellStyle(titleStyle);
        //合并单元格
        sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 2));
    }

    /**
     * 创建表头行数据
     *
     * @param workbook
     * @param sheet
     */
    public static void genExcelHeaderRow(HSSFWorkbook workbook, HSSFSheet sheet) {
        //设置表头样式
        HSSFCellStyle headerStyle = workbook.createCellStyle();
//        headerStyle.setFillBackgroundColor(IndexedColors.YELLOW.index);
        headerStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);

        HSSFRow r1 = sheet.createRow(1);
        HSSFCell c0 = r1.createCell(0);
        c0.setCellValue("用户ID");
        c0.setCellStyle(headerStyle);
        HSSFCell c1 = r1.createCell(1);
        c1.setCellValue("用户姓名");
        c1.setCellStyle(headerStyle);
        HSSFCell c2 = r1.createCell(2);
        c2.setCellValue("访问次数");
        c2.setCellStyle(headerStyle);
    }

    /**
     * 创建数据行
     *
     * @param workbook
     * @param sheet
     */
    public static void genExcelDataRow(HSSFWorkbook workbook, HSSFSheet sheet, List<UserDataDownload> dataList) {
        //设置数据样式
        HSSFCellStyle dataStyle = workbook.createCellStyle();
//        dataStyle.setFillBackgroundColor(IndexedColors.YELLOW.index);
        dataStyle.setFillPattern(FillPatternType.SOLID_FOREGROUND);
        Integer rowindex = 2;
        Integer totalColumn = 3;
        for (int i = 0; i < dataList.size(); i++) {
            UserDataDownload data = dataList.get(i);
            HSSFRow row = sheet.createRow(i + rowindex);

            HSSFCell c0 = row.createCell(0);
            c0.setCellValue(data.getIuser());
            c0.setCellStyle(dataStyle);

            HSSFCell c1 = row.createCell(1);
            c1.setCellValue(data.getUsername());
            c1.setCellStyle(dataStyle);

            HSSFCell c2 = row.createCell(2);
            c2.setCellValue(data.getVisits());
            c2.setCellStyle(dataStyle);
        }
    }


    /**
     * 数据转换
     *
     * @param workbook
     * @return
     */
    private static ResponseEntity<byte[]> dataFormatConversion(HSSFWorkbook workbook, Date[] createDateScope) {
        String title = createDateScope[0] + "_" + createDateScope[1] + " 用户访问数据表";
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        HttpHeaders headers = new HttpHeaders();
        try {
            headers.setContentDispositionFormData("attachment", new String((title + ".xls").getBytes("UTF-8"), "ISO-8859-1"));
            headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);
            workbook.write(baos);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ResponseEntity<byte[]> responseEntity = new ResponseEntity<>(baos.toByteArray(), headers, HttpStatus.CREATED);
        return responseEntity;
    }

}
