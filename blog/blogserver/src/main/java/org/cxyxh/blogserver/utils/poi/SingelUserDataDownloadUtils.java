...
public class SingelUserDataDownloadUtils {
    ...
    public static ResponseEntity<byte[]> userDataDownload(List<SingelUserDataDownload> data, Date[] createDateScope, String name) {
        username = name;
        //0.先把日期格式化
    ...
    public static void genExcelBaseInfo(HSSFWorkbook workbook) {
        ...
        //3.1文档类别 文档管理员 公司
        docInfo.setCategory("用户["+username+"]登录数据表");
        docInfo.setManager("程序员小黄");
        docInfo.setCompany("org.cxyxh");
        //4.获取文档摘要信息
        SummaryInformation summInfo = workbook.getSummaryInformation();
        //4.1 文档标题 文档备注
        summInfo.setTitle("用户_"+username+"_登录数据表");
     ...
     */
    private static void genExcelSheet(HSSFWorkbook workbook, List<SingelUserDataDownload> data, String[] dateStringArray) {
        HSSFSheet sheet = workbook.createSheet("用户登录数据表");
        //0.设置宽度
    ...
    public static void genExcelTitleRow(HSSFWorkbook workbook, HSSFSheet sheet, String[] dateStringArray) {
        ...
        String title = dateStringArray[0] + "_" + dateStringArray[1] + " 用户["+username+"]登录数据表";
    ...
    public static void genExcelDataRow(HSSFWorkbook workbook, HSSFSheet sheet, List<SingelUserDataDownload> dataList) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            c2.setCellValue(data.getIp());
            c2.setCellStyle(dataStyle);

            HSSFCell c3 = row.createCell(3);
            c3.setCellValue(data.getProvince());
            c3.setCellStyle(dataStyle);

            HSSFCell c4 = row.createCell(4);
            c4.setCellValue(data.getCity());
            c4.setCellStyle(dataStyle);

            HSSFCell c5 = row.createCell(5);
            c5.setCellValue(data.getGmtCreate());
